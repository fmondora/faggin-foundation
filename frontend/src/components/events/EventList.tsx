...
const LOCALE_MAP: Record<string, string> = { it: 'it-IT', en: 'en-US', de: 'de-DE', es: 'es-ES' };

function formatDate(date: string, locale: string): string {
  try {
    return new Date(date).toLocaleDateString(LOCALE_MAP[locale] || locale, { year: 'numeric', month: 'short' });
  } catch { return date; }
}
...
export default function EventList({ events }: { events?: any[] }) {
  ...
  const upcoming = allEvents.filter((e: any) => e.type === 'upcoming');
  const past = allEvents.filter((e: any) => e.type === 'past');
            ...
            {upcoming.map((event: any, i: number) => (
              <div key={i} className="bg-primary/5 border-l-4 border-primary p-4 rounded-r">
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-primary font-bold text-sm">{formatDate(event.date, locale)}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div key={i} className="border-b border-border pb-4">
            <div className="flex justify-between items-start">
              ...
              <div className="flex items-center gap-3">
                <span className="text-text-light text-sm">{formatDate(event.date, locale)}</span>
