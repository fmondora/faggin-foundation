...
export default function VideoThemeSection({ themes }: { themes?: any[] }) {
  const displayThemes = themes?.length ? themes : FALLBACK_THEMES;

  return (
    <>
      {displayThemes.map((theme: any, i: number) => (
        <section key={theme.name || i} className={`py-12 px-4 ${i % 2 === 0 ? 'bg-bg-alt' : 'bg-bg'}`}>
          <div className="max-w-[1200px] mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {(theme.videos || []).map((video: any) => <VideoCard key={video.youtubeId} video={video} />)}
            </div>
          </div>
        </section>
      ))}
